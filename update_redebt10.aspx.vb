...
Partial Class update_redebt10
    Inherits System.Web.UI.Page
    Dim DB105 As New Cls_Data105
    Dim CP As New Cls_Panu
    Dim CF As New Cls_RequestFlow

    Dim pageUrl As String = "redebt10"
    ...
    Sub loadDetail(ByVal vRequest_id As String)
        ...
        If(vDT.Rows().Count() > 0)
            ...
            txt_dx01.Value() = vDT.Rows(0).Item("dx01") 'วันที่ชำระผิด
            'txt_dx02.Value() = vDT.Rows(0).Item("dx02")
            txt_dx03.Value() = vDT.Rows(0).Item("dx03") 'วันที่ลูกค้าขอลดหนี้

            'txt_mx01.Value() = vDT.Rows(0).Item("mx01")
            'txt_mx02.Value() = vDT.Rows(0).Item("mx02")
            'txt_mx03.Value() = vDT.Rows(0).Item("mx03")

            'txt_tx01.Value() = vDT.Rows(0).Item("tx01")
            'txt_tx02.Value() = vDT.Rows(0).Item("tx02")
            'txt_tx03.Value() = vDT.Rows(0).Item("tx03")
            ...
            inn_dx01.InnerHtml = vDT.Rows(0).Item("dx01") 'วันที่ชำระผิด
            'inn_dx02.InnerHtml = vDT.Rows(0).Item("dx02")
            inn_dx03.InnerHtml = vDT.Rows(0).Item("dx03") 'วันที่ลูกค้าขอลดหนี้

            'inn_mx01.InnerHtml = vDT.Rows(0).Item("mx01")
            'inn_mx02.InnerHtml = vDT.Rows(0).Item("mx02")
            'inn_mx03.InnerHtml = vDT.Rows(0).Item("mx03")
    ...
    Sub Update_Submited()
        ...
        Dim dx01 As String = CP.rReplaceQuote(txt_dx01.Value) 'วันที่ชำระผิด
        ...
        Dim mx01 As String = ""'CP.rReplaceQuote(txt_mx01.Value)
        Dim mx02 As String = ""'CP.rReplaceQuote(txt_mx02.Value)
        Dim mx03 As String = ""'CP.rReplaceQuote(txt_mx03.Value)

        Dim tx01 As String = ""'CP.rReplaceQuote(txt_tx01.Value)
