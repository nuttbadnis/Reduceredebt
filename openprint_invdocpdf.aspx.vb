...
Partial Class openprint_invdocpdf
        ...
        If (vDT.Rows().Count() > 0) Then
            ...
            Else
                ...
                Else If payment_value = 40 Then
                    ...
                    receipt_footer += "</th>"
                    receipt_footer += "<th valign='top' style='text-align: left;font-size: 16px;'>"
                    receipt_footer += "[ / ] &nbsp;เช็คธนาคาร &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+ vDT.Rows(0).Item("real_title") + " สาขา " + vDT.Rows(0).Item("mx03") 
                    receipt_footer += "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;เลขที่เช็ค &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+vDT.Rows(0).Item("fx01")+" ลว." + vDT.Rows(0).Item("dx04")
                End If
            ...
            hide_request_remark.Value() = vDT.Rows(0).Item("tx03")
            hide_request_title.Value() = vDT.Rows(0).Item("request_title")

            Dim Request_Title as string = hide_request_title.Value
            Dim SearchForThis As String = "TTTBB"
            Dim FirstCharacter As Integer = Request_Title.IndexOf(SearchForThis)

            Dim cell_space as String = "" 
            If vCategory_id = "1" Then 'ใบแจ้งหนี้
                
                ' If FirstCharacter >= 0 Then
                '     inn_create_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_2.png' style='width: 80px;height: 50px;padding-bottom: 8px;'/>"
                '     inncp_create_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_2.png' style='width: 80px;height: 50px;padding-bottom: 8px;'/>"     
                ' Else
                '     inn_create_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_1.png' style='width: 100px;height: 50px;padding-bottom: 7px;'/>"
                '     inncp_create_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_1.png' style='width: 100px;height: 50px;padding-bottom: 7px;'/>"
                ' End If
                inn_create_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_1.png' style='width: 100px;height: 50px;padding-bottom: 7px;'/>"
                inncp_create_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_1.png' style='width: 100px;height: 50px;padding-bottom: 7px;'/>"
                inn_received_sign.innerhtml = "<br/>"
                inncp_received_sign.innerhtml = "<br/>"
            Else 'ใบเสร็จ
                ' If FirstCharacter >= 0 Then
                '     inn_create_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_2.png' style='width: 80px;height: 50px;padding-bottom: 8px;'/>"
                '     inncp_create_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_2.png' style='width: 80px;height: 50px;padding-bottom: 8px;'/>"
                '     inn_received_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_2.png' style='width: 80px;height: 50px;padding-bottom: 8px;'/>"
                '     inncp_received_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_2.png' style='width: 80px;height: 50px;padding-bottom: 8px;'/>"      
                ' Else
                    inn_create_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_1.png' style='width: 100px;height: 50px;padding-bottom: 7px;'/>"
                    inncp_create_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_1.png' style='width: 100px;height: 50px;padding-bottom: 7px;'/>"
                    inn_received_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_1.png' style='width: 100px;height: 50px;padding-bottom: 7px;'/>"
                    inncp_received_sign.innerhtml = "<img src='App_Inc/_img/signature_prepare_1.png' style='width: 100px;height: 50px;padding-bottom: 7px;'/>"
                ' End If
            End If
    ...
    Protected Sub ExportToPDF(sender As Object, e As EventArgs)
        ...
        Dim path_sign_metinee As String = Server.MapPath(".") + "/App_Inc/_img/signature_approve.png"
        Dim path_sign_waranthita As String = Server.MapPath(".") + "/App_Inc/_img/signature_prepare_1.png"
        Dim path_sign_supansa As String = Server.MapPath(".") + "/App_Inc/_img/signature_prepare_2.png"

        Dim img_sign_metinee As iTextSharp.text.Image = iTextSharp.text.Image.GetInstance(path_sign_metinee)
        Dim img_sign_waranthita As iTextSharp.text.Image = iTextSharp.text.Image.GetInstance(path_sign_waranthita)
        Dim img_sign_supansa As iTextSharp.text.Image = iTextSharp.text.Image.GetInstance(path_sign_supansa)

        img_sign_metinee.ScaleAbsolute(60.0F, 20.0F)
        img_sign_waranthita.ScaleAbsolute(60.0F, 25.0F)
        img_sign_supansa.ScaleAbsolute(60.0F, 25.0F)

        Dim Request_Title as string = hide_request_title.Value
        Dim SearchForThis As String = "TTTBB"
        Dim FirstCharacter As Integer = Request_Title.IndexOf(SearchForThis)
        Dim Request_Category as string = hide_category_id.Value
        ...
        If Request_Category = 1 Then 'ใบแจ้งหนี้
            ' If FirstCharacter >= 0 Then
            '     cell_col1_row1 = New PdfPCell(img_sign_supansa)       
            ' Else
            '     cell_col1_row1 = New PdfPCell(img_sign_waranthita)
            ' End If

            cell_col1_row2 = New PdfPCell(new Phrase(""))
        Else 'ใบเสร็จ
            ' If FirstCharacter >= 0 Then
            '     cell_col1_row1 = New PdfPCell(img_sign_supansa)
            '     cell_col1_row2 = New PdfPCell(img_sign_supansa)       
            ' Else
                ' cell_col1_row1 = New PdfPCell(img_sign_waranthita)
                ' cell_col1_row2 = New PdfPCell(img_sign_waranthita)
            ' End If
            cell_col1_row2 = New PdfPCell(img_sign_waranthita)
        End If
        cell_col1_row1 = New PdfPCell(img_sign_waranthita)
        cell_col1_row3 = New PdfPCell(img_sign_metinee)
