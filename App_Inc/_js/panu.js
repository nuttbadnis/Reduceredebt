function stopIE(){
	if(navigator.userAgent.indexOf('MSIE')>0){  
		$('#txt_loading').html(" <b>คุณกำลังใช้โปรแกรมที่ไม่สนับสนุน กรุณาใช้ Chrome หรือ Firefox<b>");
		$('#page_loading').fadeIn();
	}
		return;
}
...
function checkSession() {
	...
	if($('input[xd="hide_uemail"]').val() != null) {
		if($('input[xd="hide_uemail"]').val().length > 0){
			hide_uemail = true;
		}
    }
...
function setDatePicker() {
	...
	$('input[xd="txt_dx01"]').prop('readonly', true).datepicker("destroy");
}
