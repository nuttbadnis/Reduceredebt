...
Partial Class update_invdoc25
    Inherits System.Web.UI.Page
    Dim DB105 As New Cls_Data105
    Dim CP As New Cls_Panu
    Dim CF As New Cls_RequestFlow

    Dim pageUrl As String = "update_invdoc25"
    ...
    Sub loadDetail(ByVal vRequest_id As String)
        ...
        If(vDT.Rows().Count() > 0)
            ...
            Dim inn_request_file2 as string = "<br>" + request_file2
            ...
            loadInvFile(vRequest_id, vDT.Rows(0).Item("request_status"), vDT.Rows(0).Item("request_step")) '<< invdoc_form by panu
    ...
    Sub loadInvFile(ByVal vRequest_id As String, ByVal vRequest_status As String, ByVal vRequest_step As String) '<< invdoc_form by panu
        If CF.checkCanEditInvFile(vRequest_id, vRequest_status, vRequest_step) = 1 Then
            ' โหลดใบแจ้งหนี้ เพื่อไปวางบิล เสร็จแล้วกลับมาอัพโหลด 
            view_invfile.Style.Add("display", "none")
            edit_invfile.Style.Add("display", "block")
            none_invfile.Style.Add("display", "none")

        Else
            btn_print_pdf1.Visible = False
            view_invfile.Style.Add("display", "block")
            edit_invfile.Style.Add("display", "none")
            none_invfile.Style.Add("display", "none")
        End If
    End Sub

    Sub Invfile_Submit(ByVal Source As Object, ByVal E As EventArgs) '<< invdoc_form by panu
        Dim xRequest_id = Request.QueryString("request_id")
        Dim update_by As String = hide_uemail.Value
        Dim tx02 As String = CP.rReplaceQuote(txt_invfile.Value)

        CF.UpdateInvDocfile(xRequest_id, update_by, tx02)
    End Sub
